...
int init_delete(int ch) {
    for(int i = 0; i < ch; i++) {
        ...
        if (not_enough->num_of_keys >= check){ //* dogyeom
            ...
            else if(parent->b_f[0].p_offset == checker[i]) {
                neighbor_index = -1;
...
int db_delete2(int64_t key) {
    ...
    free(check);
    off_t deloff = find_leaf(key);
    delete_entry2(key, deloff);
    return 0;

}//fin

void delete_entry2(int64_t key, off_t deloff) {
...
void coalesce_pages(off_t will_be_coal, int nbor_index, off_t nbor_off, off_t par_off, int64_t k_prime) {
    ...
    //ch가 지정되지 않았으므로
    //이부분을 수정했을 때 더욱 빨라질 수도 있으리라 생각함
    delete_entry2(k_prime, par_off); //* dogyeom
    free(wbc);
    usetofree(wbf);
    free(nbor);
    free(parent);
