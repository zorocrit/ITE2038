...
<body>
    <h3 style="test-align: center;"> 교과목명 검색</h3>
    ...
    <table width="300" border="1">
        <tr>
            ...
            try {
                String lec = request.getParameter("lectureName");
                String sql = "select class.class_id, class_no, class.person_max, course_id, count(time.period) as timePeriod, class.name, lecturer.name, group_concat(time.begin) as timeInfo, class.room_id, count(distinct application.idapplication) as counting, building.name from class left join lecturer on lecturer.lecturer_id = class.lecturer_id left join time on time.class_id = class.class_id left join room on room.room_id = class.room_id left join building on building.building_id = room.building_id left join application on class.class_id = application.class_id where begin is not null and class.name like '%" + lec + "%' group by class.class_id";
