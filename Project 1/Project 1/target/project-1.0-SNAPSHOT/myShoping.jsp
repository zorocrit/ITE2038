...
<%
    ...
    %>
    <h3 style="test-align: center;"> 희망 수업</h3>
    <table id = "listTable" width="300" border="1">
        ...
        <%ResultSet rs = null;
            ...
            try {
                String sql = "select class.class_id, class.person_max, class_no, course_id, count(time.period) as timePeriod, class.name, lecturer.name, group_concat(time.begin) as timeInfo, class.room_id, building.name from class left join lecturer on lecturer.lecturer_id = class.lecturer_id left join time on time.class_id = class.class_id left join room on room.room_id = class.room_id left join building on building.building_id = room.building_id left join shopping on shopping.class_id = class.class_id where begin is not null and student_id = '" + userID + "' group by class.class_id";
...
%>
        <tr>
            ...
            <form method="post" action="deleteMyHopeLecture.jsp" >
                <td><input type = "checkbox" value="<%=cid%>" id = "wanaDelLec" name = "wanaDelLec"></td>
                <td><input type = "submit" value="확인"></td>
            </form>
            <td><%=class_no%></td>
            <td><%=course_id%></td>
